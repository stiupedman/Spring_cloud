...
-- 初始化-部门表数据
-- ----------------------------
insert into sys_dept values(100,  0,   '0',          '家教服务',   0, 'ljh', '15888888888', 'ljh@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(101,  100, '0,100',      '管理处',     1, 'ljh', '15888888888', 'ljh@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(102,  100, '0,100',      '教师处',     2, 'ljh', '15888888888', 'ljh@qq.com', '0', '0', 'admin', sysdate(), '', null);
insert into sys_dept values(103,  100, '0,100',      '学生处',     3, 'ljh', '15888888888', 'ljh@qq.com', '0', '0', 'admin', sysdate(), '', null);
-- ----------------------------
-- 职级表
-- ----------------------------
drop table if exists sys_rank;
create table sys_rank (
                          rank_id           bigint(20)      not null auto_increment    comment '职级ID',
                          rank_code         varchar(64)     not null                   comment '职级编码',
                          rank_name         varchar(50)     not null                   comment '职级名称',
                          rank_sort         int(4)          not null                   comment '显示顺序',
                          ...
                          primary key (rank_id)
) engine=innodb comment = '职级表';

-- ----------------------------
-- 初始化-职级表数据
-- ----------------------------
insert into sys_rank values(1, 'R01', '小学',   1, '0', 'admin', sysdate(), '', null, '小学');
insert into sys_rank values(2, 'R02', '初中',   2, '0', 'admin', sysdate(), '', null, '初中');
insert into sys_rank values(3, 'R03', '高中',   3, '0', 'admin', sysdate(), '', null, '高中');
...
create table sys_user (
    ...
    -- 新增职级及角色相关字段
                          ...
                          role_names        varchar(200)    default ''                 comment '角色名称集合（冗余）',
                          role_ids          varchar(100)    default ''                 comment '角色ID集合（冗余）',
                          primary key (user_id),
                          constraint fk_user_rank foreign key (rank_id) references sys_rank (rank_id)
...
create table sys_student (
                             ...
                             salary_expect           char(1)         default '0'                comment '薪资期望',
...
create table sys_parent (
                            ...
                            salary_reward           char(1)         default '0'                comment '薪资报酬',
                            auth_status          char(1)         default '0'                comment '认证状态（0代表未认证 1表示已认证）',
                            del_flag          char(1)         default '0'                comment '删除标志（0代表存在 2代表删除）',
                            create_by         varchar(64)     default ''                 comment '创建者',
                            create_time       datetime                                   comment '创建时间',
...
-- ----------------------------
-- 触发器：仅限制管理员不能绑定职级，保留管理权限
-- ----------------------------
DELIMITER $$
CREATE TRIGGER trg_check_admin_rank BEFORE UPDATE ON sys_user
    FOR EACH ROW
BEGIN
    -- 仅限制角色ID为1（超级管理员）或2（管理员）时不能绑定职级
    IF (NEW.role_ids LIKE '%,1%' OR NEW.role_ids = '1' OR
      NEW.role_ids LIKE '%,2%' OR NEW.role_ids = '2') AND
     NEW.rank_id IS NOT NULL THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = '管理员和超级管理员角色不能绑定职级';
END IF;
END$$
DELIMITER ;

DELIMITER $$
CREATE TRIGGER trg_check_admin_rank_insert BEFORE INSERT ON sys_user
    FOR EACH ROW
BEGIN
    IF (NEW.role_ids LIKE '%,1%' OR NEW.role_ids = '1' OR
      NEW.role_ids LIKE '%,2%' OR NEW.role_ids = '2') AND
...
-- ----------------------------
-- 初始化-用户信息表数据（管理员无职级但保留管理权限）
-- ----------------------------
-- 超级管理员（无职级，保留管理权限）
insert into sys_user values(1,  101, 'admin', '老了', '00', 'ry@163.com', '15888888888', '1', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', sysdate(), sysdate(), 'admin', sysdate(), '', null, '管理员', NULL, '超级管理员', '1');

-- 普通管理员（无职级，保留管理权限）
insert into sys_user values(3,  101, 'ljh',    '小了', '00', 'ry@qq.com',  '15666666666', '1', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', sysdate(), sysdate(), 'admin', sysdate(), '', null, '数学', NULL, '管理员', '2');

-- 教师用户（有职级）
insert into sys_user values(4,  102, 'teacher1', '王老师', '00', 'teacher@qq.com',  '15999999999', '1', '', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', sysdate(), sysdate(), 'admin', sysdate(), '', null, '语文教师', 3, '教师', '3');
...
insert into sys_post values(2, 'Math',   '数学',  2, '0', 'admin', sysdate(), '', null, '');
insert into sys_post values(3, 'Chinese',   '语文',  3, '0', 'admin', sysdate(), '', null, '');
insert into sys_post values(4, 'English', '英语',  4, '0', 'admin', sysdate(), '', null, '');
...
insert into sys_role values('3', '教师',    'teacher', 3, 1, 1, 1, '0', '0', 'admin', sysdate(), '', null, '教师');
insert into sys_role values('4', '学生',    'student', 4, 1, 1, 1, '0', '0', 'admin', sysdate(), '', null, '学生');
...
insert into sys_menu values('4', '代码地址', '0', '4', 'https://gitee.com/Ljhao2004/spring_cloud_first', null, '', '', 0, 0, 'M', '0', '0', '', 'guide',    'admin', sysdate(), '', null, '若依官网地址');
-- 二级菜单
...
-- 新增职级管理
insert into sys_menu values(117,  '职级管理',       '1',   '10', 'rank',       'system/rank/index',                 '', '', 1, 0, 'C', '0', '0', 'system:rank:list',        'rank',          'admin', sysdate(), '', null, '职级管理菜单');
-- 三级菜单
...
-- 职级管理按钮
insert into sys_menu values(1061, '职级查询', '117', '1',  '', '', '', '', 1, 0, 'F', '0', '0', 'system:rank:query',          '#', 'admin', sysdate(), '', null, '');
insert into sys_menu values(1062, '职级新增', '117', '2',  '', '', '', '', 1, 0, 'F', '0', '0', 'system:rank:add',            '#', 'admin', sysdate(), '', null, '');
insert into sys_menu values(1063, '职级修改', '117', '3',  '', '', '', '', 1, 0, 'F', '0', '0', 'system:rank:edit',           '#', 'admin', sysdate(), '', null, '');
insert into sys_menu values(1064, '职级删除', '117', '4',  '', '', '', '', 1, 0, 'F', '0', '0', 'system:rank:remove',         '#', 'admin', sysdate(), '', null, '');
insert into sys_menu values(1065, '职级导出', '117', '5',  '', '', '', '', 1, 0, 'F', '0', '0', 'system:rank:export',         '#', 'admin', sysdate(), '', null, '');
insert into sys_menu values(1066, '角色关联', '117', '6',  '', '', '', '', 1, 0, 'F', '0', '0', 'system:rank:role:relation',  '#', 'admin', sysdate(), '', null, '');
...
-- ----------------------------
-- 4、职级与角色关联表（仅限制角色绑定，不限制管理权限）
-- ----------------------------
drop table if exists sys_rank_role;
create table sys_rank_role (
                               rank_id   bigint(20) not null comment '职级ID',
                               role_id   bigint(20) not null comment '角色ID（仅允许教师、学生角色）',
                               primary key(rank_id, role_id),
                               constraint fk_rank_role_id foreign key (role_id) references sys_role (role_id),
    -- 仅限制不能关联管理员角色，不限制管理员操作权限
                               constraint chk_role_not_admin check (role_id not in (1, 2))
) engine=innodb comment = '职级与角色关联表（仅关联教师、学生角色）';

-- ----------------------------
-- 初始化-职级与角色关联表数据（仅关联教师、学生角色）
-- ----------------------------
-- 小学职级关联教师和学生角色
insert into sys_rank_role values (1, 3);  -- 小学 -> 教师角色
insert into sys_rank_role values (1, 4);  -- 小学 -> 学生角色
-- 初中职级关联教师和学生角色
insert into sys_rank_role values (2, 3);  -- 初中 -> 教师角色
insert into sys_rank_role values (2, 4);  -- 初中 -> 学生角色
-- 高中职级关联教师和学生角色
insert into sys_rank_role values (3, 3);  -- 高中 -> 教师角色
insert into sys_rank_role values (3, 4);  -- 高中 -> 学生角色
...
-- 管理员角色关联职级管理菜单及按钮
insert into sys_role_menu values ('2', '117');   -- 职级管理菜单
insert into sys_role_menu values ('2', '1061');  -- 职级查询
insert into sys_role_menu values ('2', '1062');  -- 职级新增
insert into sys_role_menu values ('2', '1063');  -- 职级修改
insert into sys_role_menu values ('2', '1064');  -- 职级删除
insert into sys_role_menu values ('2', '1065');  -- 职级导出
insert into sys_role_menu values ('2', '1066');  -- 角色关联
-- ----------------------------
-- 8、角色和部门关联表  角色1-N部门
-- ----------------------------
